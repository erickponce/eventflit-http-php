...
class Eventflit implements LoggerAwareInterface
{
     ...
     * PHP5 Constructor.
     *
     * Initializes a new Eventflit instance with key, secret , app ID and channel.
     ...
     *                         Options to configure the Eventflit instance.
    ...
    public function __construct($auth_key, $secret, $app_id, $options = array(), $host = null, $port = null, $timeout = null)
    {
        ...
        if (isset($options['notification_host'])) {
            ...
            $this->settings['notification_host'] = 'push.eventflit.com';
        }
        ...
        if (!array_key_exists('host', $this->settings)) {
            if (array_key_exists('host', $options)) {
                ...
                $this->settings['host'] = 'api-'.$options['cluster'].'.eventflit.com';
            } else {
                $this->settings['host'] = 'service.eventflit.com';
            }
        }
     ...
     *
     * @deprecated Use the PSR-3 compliant Eventflit::setLogger() instead. This method will be removed in the next breaking release.
     *
     * @return void
     */
     ...
     *
     * @throws EventflitException if any required dependencies are missing
     *
     * @return void
     */
    private function check_compatibility()
    {
        if (!extension_loaded('curl')) {
            throw new EventflitException('The Eventflit library requires the PHP cURL module. Please ensure it is installed');
        }

        if (!extension_loaded('json')) {
            throw new EventflitException('The Eventflit library requires the PHP JSON module. Please ensure it is installed');
        }

        if (!in_array('sha256', hash_algos())) {
            throw new EventflitException('SHA256 appears to be unsupported - make sure you have support for it, or upgrade your version of PHP.');
        }
    }
     ...
     *
     * @throws EventflitException if $channels is too big or any channel is invalid
     *
     * @return void
     */
    private function validate_channels($channels)
    {
        if (count($channels) > 100) {
            throw new EventflitException('An event can be triggered on a maximum of 100 channels in a single call.');
        }
     ...
     *
     * @throws EventflitException if $channel is invalid
     *
     * @return void
     */
    private function validate_channel($channel)
    {
        if (!preg_match('/\A[-a-zA-Z0-9_=@,.;]+\z/', $channel)) {
            throw new EventflitException('Invalid channel name '.$channel);
        }
    }
     ...
     *
     * @throws EventflitException if $socket_id is invalid
     */
    private function validate_socket_id($socket_id)
    {
        if ($socket_id !== null && !preg_match('/\A\d+\.\d+\z/', $socket_id)) {
            throw new EventflitException('Invalid socket ID '.$socket_id);
        }
    }
    ...
    private function create_curl($domain, $s_url, $request_method = 'GET', $query_params = array())
    {
        ...
        if ($this->ch === false) {
            throw new EventflitException('Could not initialise cURL!');
        }
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            ...
            'Expect:',
            'X-Eventflit-Library: eventflit-http-php '.self::$VERSION,
        ));
     ...
     * @param params array API params (see http://eventflit.com/docs/rest_api)
     *
     * @return See Eventflit API docs
     */
     ...
     * @param bool  $debug
     *
     * @throws EventflitException if validation fails.
     *
     * @return bool|string
     */
    public function notify($interests, $data = array(), $debug = false)
    {
        ...
        if (count($interests) === 0) {
            throw new EventflitException('$interests array must not be empty');
        }
